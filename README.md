...
- **server/.env**
  ...
  - **Database**: `DATABASE_SCHEMA` always wins for the schema/database name, overriding both the path embedded in `DATABASE_URL` and any `DB_NAME` fallback. Use `DATABASE_URL` for a full DSN; when omitted, `DB_HOST`/`DB_PORT`/`DB_USER`/`DB_PASSWORD` combine with `DATABASE_SCHEMA` (or `DB_NAME` if no schema override) to reach MySQL. These values appear in the admin settings page so operators can verify connectivity quickly.
  ...
  - **CORS**: `ALLOWED_ORIGINS` lists comma-separated client origins permitted to call the API. The server refuses to start when this variable is empty so you can wire approved domains before exposing the service. Supply explicit origins for every environment (for example, `http://localhost:4173,http://localhost:5173,http://localhost:3000` to match the admin dev server and other local clients during development or `https://admin.example.com,https://app.example.com` in production/staging).
