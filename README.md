...
- **admin/.env**
  - **API endpoints**: `VITE_API_BASE_URL`, `VITE_API_WEBSOCKET_URL`, `VITE_TILE_CDN_URL`, `VITE_MAP_PACKAGE_BASE_URL`, `VITE_MAP_PACKAGE_INDEX_URL`, `VITE_MAP_PACKAGE_DOWNLOAD_URL`, and `VITE_MAP_PACKAGE_MANIFEST_URL` populate the settings form so the admin UI talks to the right HTTP, websocket, and tile/map download hosts. These are required; `node admin/build.js` stops and lists any missing/blank values.
  - **Auth**: `VITE_AUTH_DOMAIN`, `VITE_AUTH_CLIENT_ID`, `VITE_AUTH_AUDIENCE`, `VITE_AUTH_SCOPE`, `VITE_AUTH_REDIRECT_URI`, and `VITE_AUTH_POST_LOGOUT_REDIRECT_URI` configure the OIDC login flow the settings screen exposes to administrators. The form also auto-populates the explicit `OIDC_*` fields (`OIDC_AUTHORIZE_URL`, `OIDC_TOKEN_URL`, `OIDC_CLIENT_ID`, `OIDC_SCOPE`, `OIDC_AUDIENCE`, `OIDC_REDIRECT_URI`) exported by `node admin/build.js` from the admin env file.
  - **UI defaults**: `VITE_SETTINGS_DEFAULT_REGION`, `VITE_SETTINGS_DEFAULT_MAP_STYLE`, and `VITE_SETTINGS_FEATURE_FLAGS` let the single settings page toggle UI options (e.g., map style or feature switches) without hard-coding them. They are validated as required inputs during the admin build.
  ...
  - **Build/export behavior**: Running `node admin/build.js` reads `admin/.env` (plus overrides in `admin/.env.local`) and exports only an explicit allowlist of `VITE_*` keys (API endpoints, auth wiring, defaults, and branding) plus `API_BASE_URL`, stripping the `VITE_` prefix before writing them to `window.*` in `admin/dist/env.js` so the settings form (including region/map defaults and feature flags) auto-populates on first load. Process-level `VITE_*` variables are ignored to prevent accidental leakage from CI/production environments.
- **server/.env**
  ...
  - **Auth/JWT**: `OIDC_ISSUER`, `OIDC_AUDIENCE`, `OIDC_JWKS_URL`, and `OIDC_JWT_SECRET` are the variables the auth middleware reads. For RS256 tokens, supply your issuer plus a `OIDC_JWKS_URL` (or discovery URL) so the server can fetch and rotate signing keys. For HS256 tokens, provide a long, random `OIDC_JWT_SECRET` you control end-to-end (32+ characters) and avoid sharing it with untrusted systems. `OIDC_REVOCATION_URL` powers the admin logout endpoint; pair it with `OIDC_CLIENT_ID`/`OIDC_CLIENT_SECRET` if your identity provider requires client authentication for revocation. `AUTH_DOMAIN`, `AUTH_CLIENT_ID`, `AUTH_CLIENT_SECRET`, and `OIDC_DISCOVERY_URL` remain available for IdP integrations exposed in the settings page.
  - **CORS**: `ALLOWED_ORIGINS` lists comma-separated client origins permitted to call the API. The server refuses to start in production when this variable is empty so you can wire approved domains before exposing the service. In development, a safe default of `http://localhost:5173,http://localhost:3000` is applied automatically if `ALLOWED_ORIGINS` is unset; override it with your own list as needed. For production, set `ALLOWED_ORIGINS` to the fully qualified origins that host your admin and client apps (for example, `https://admin.example.com,https://app.example.com`).
...
6. Run the admin panel: `cd admin && npm run dev` to build `admin/dist` and serve it locally on port `4173` via the bundled Node-based static server.
