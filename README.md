...
- **admin/.env**
  ...
  - **Build/export behavior**: Running `node admin/build.js` reads `admin/.env` (plus overrides in `admin/.env.local`) and whitelists all of the `VITE_*` keys above (plus `API_BASE_URL`), stripping the prefix before exporting them to `window.*` in `admin/dist/env.js` so the settings form auto-populates on first load. The allowlist is generated from `admin/.env.example`, so any newly documented `VITE_` keys in that template are exported automatically.
- **server/.env**
  ...
  - **CORS**: `ALLOWED_ORIGINS` lists comma-separated client origins permitted to call the API. This variable is **required in production**—the server rejects requests (and surfaces an OPTIONS error) when it is missing. For local admin/mobile testing, set `ALLOWED_ORIGINS=http://localhost:5173,http://localhost:3000` so both dev servers can reach the backend without extra configuration.
