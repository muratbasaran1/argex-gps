...
- **server/.env**
  ...
  - **CORS**: `ALLOWED_ORIGINS` lists comma-separated client origins permitted to call the API. The server refuses to start when this variable is empty so you can wire approved domains before exposing the service. Supply explicit origins for every environment (for example, `http://localhost:5173,http://localhost:3000` during development or `https://admin.example.com,https://app.example.com` in production/staging).
