...
- **server/.env**
  ...
  - **CORS**: `ALLOWED_ORIGINS` lists comma-separated client origins permitted to call the API. The server refuses to start in production when this variable is empty so you can wire approved domains before exposing the service. In development, you can opt into a safe fallback of `http://localhost:5173,http://localhost:3000` by setting `ENABLE_DEV_CORS_FALLBACK=true`; otherwise supply your own `ALLOWED_ORIGINS` list. For production, set `ALLOWED_ORIGINS` to the fully qualified origins that host your admin and client apps (for example, `https://admin.example.com,https://app.example.com`).
