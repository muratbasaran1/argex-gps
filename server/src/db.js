...
async function ensureSettingsSchema(pool) {
  ...
  if (descriptionColumn && descriptionColumn[0]?.Type?.toLowerCase().includes('text')) {
    await pool.execute('ALTER TABLE settings MODIFY COLUMN description VARCHAR(256) DEFAULT ""');
  }
